...
import { BUSINESS, getThemePath, PRIORITY_THEME_ROUTES, slugify, WHATSAPP_URL } from "@/lib/business";
...
function PageHero({ eyebrow, title, lead, image }: { eyebrow: string; title: string; lead: string; image?: string }) {
  return <section className="seo-hero"><div className="seo-hero__glow" /><div className="seo-content seo-hero__layout"><div className="seo-hero__copy"><p className="seo-eyebrow"><Sparkles size={14} />{eyebrow}</p><h1>{title}</h1><p>{lead}</p><div className="seo-hero__actions"><a className="seo-button seo-button--primary" href={WHATSAPP_URL} target="_blank" rel="noreferrer">Consultar no WhatsApp <MessageCircle size={17} /></a><Link className="seo-button seo-button--secondary" href="/#temas">Ver temas <ArrowRight size={17} /></Link></div></div>{image && <figure className="seo-hero__photo"><img src={image} alt="Kit de decoração Jacque Pegue e Monte em Goianésia – GO" /></figure>}</div></section>;
}
...
export function ThemeDetailPage({ themeSlug }: { themeSlug: string }) {
  ...
  return <PublicPageShell><PageMeta title={title} description={`Decoração ${theme.name} Pegue & Monte em Goianésia – GO. Consulte a disponibilidade do kit para sua comemoração.`} /><section className="theme-detail"><div className="theme-detail__image"><img src={theme.image} alt={`Decoração ${theme.name} Pegue & Monte para festa em Goianésia – GO`} /></div><div className="theme-detail__copy"><p className="seo-eyebrow">Tema {theme.category}</p><h1>Decoração {theme.name}<br /><em>em Goianésia – GO</em></h1><p>Um tema do acervo Jacque Pegue & Monte para você consultar, retirar e montar na sua celebração.</p><a className="seo-button seo-button--primary" href={WHATSAPP_URL} target="_blank" rel="noreferrer">Quero consultar este tema <MessageCircle size={17} /></a><Link className="seo-text-link" href="/#temas">Voltar para todos os temas <ArrowRight size={16} /></Link></div></section><section className="seo-split theme-detail__info"><div className="seo-content seo-split__grid"><div><p className="seo-eyebrow">Como reservar</p><h2>Confirme a data e<br /><em>organize sua retirada.</em></h2><p className="seo-body">A disponibilidade e a composição do kit são confirmadas diretamente no atendimento, conforme a data da sua comemoração.</p></div><div className="seo-check-list"><p><CheckCircle2 size={18} />Consulte a disponibilidade para a data desejada.</p><p><CheckCircle2 size={18} />Alinhe retirada, devolução e as orientações com a equipe.</p><p><CheckCircle2 size={18} />Use a foto real do tema para planejar seu cenário.</p></div></div></section><section className="seo-related"><div className="seo-content"><p className="seo-eyebrow">Você também pode gostar</p><h2>Mais temas para<br /><em>continuar explorando.</em></h2><div className="seo-related__grid">{related.map((item) => <article key={item.name}><img src={item.image} alt={`Decoração ${item.name} em Goianésia – GO`} loading="lazy" /><h3>{item.name}</h3><Link href="/#temas">Ver no catálogo <ArrowRight size={15} /></Link></article>)}</div></div></section></PublicPageShell>;
}
